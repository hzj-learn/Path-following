...
#include"../include/base_vel_mux/exceptions.hpp"
...
void CmdVelMux::timerCallback(const ros::TimerEvent& event, unsigned int idx)
{
  if (cmd_vel_subs.allowed == idx || (idx == GLOBAL_TIMER && cmd_vel_subs.allowed != VACANT))
  {
    if (idx == GLOBAL_TIMER)
    {
      ...
      ROS_WARN("BaseVelMux : No cmd_vel messages from ANY input received in the last %fs", common_timer_period);
      ROS_WARN("BaseVelMux : %s dislodged due to general timeout",
...
bool CmdVelMux::init()
{
  ...
  **********************/
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<base_vel_mux::reloadConfig>(nh_);
  dynamic_reconfigure_server->setCallback(dynamic_reconfigure_cb);

  active_subscriber = nh_.advertise <std_msgs::String> ("active", 1, true); // latched topic
  ...
  ROS_INFO("CmdVelMux : successfully initialized");
  return true;
}

void CmdVelMux::reloadConfiguration(base_vel_mux::reloadConfig &config, uint32_t unused_level)
{
  ...
  else
  {
    ...
    if (config.yaml_cfg_file == "")
    {
      ...
      nh_.getParam(node_name_+"/yaml_cfg_file", yaml_cfg_file);
  ...
  **********************/
  ...
  if (output_topic_name != output_name)
  {
    ...
    output_topic_pub = nh_.advertise<geometry_msgs::Twist>(output_topic_name, 10);
  ...
  **********************/
  ...
  for (unsigned int i = 0; i < cmd_vel_subs.size(); i++)
  {
    ...
    {
      cmd_vel_subs[i]->subs =
          nh_.subscribe<geometry_msgs::Twist>(cmd_vel_subs[i]->topic, 10, CmdVelFunctor(i, this));
    ...
    if (!cmd_vel_subs[i]->timer)
    {
      ...
      cmd_vel_subs[i]->timer =
          nh_.createTimer(ros::Duration(cmd_vel_subs[i]->timeout), TimerFunctor(i, this), true, false);
    }
  ...
  if (!common_timer)
  {
    ...
    common_timer =
        nh_.createTimer(ros::Duration(common_timer_period), TimerFunctor(GLOBAL_TIMER, this), true, false);
  }
  ...
  ROS_INFO("CmdVelMux : (re)configured");
